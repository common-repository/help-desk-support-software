jQuery(document).ready(function($) {
    ...
    if (jQuery(".shd-tootltip").length > 0) {
        jQuery(".shd-tootltip").tooltip();
    }

    /* plugin activation notice dismis.*/
    jQuery(".shd-notice .notice-dismiss").on('click', function() {
        var data = {
            'action': 'shd_dismiss_notice'
        };
        jQuery.post(shd_ajax_object.ajax_url, data, function(response) {
        });
    });
});