 ...
 */
if (!function_exists('shd_dashboard_cb')) {

    function shd_dashboard_cb() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', SWIFTHD__PLUGIN_URL . 'admin/js/jstz.min.js', '', '', true);
        wp_enqueue_style('swift-dashboard', SWIFTHD__PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(SWIFTHD_PLUGIN_PREFIX . 'dashboard-script', SWIFTHD__PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);
        wp_enqueue_style('swiftcloud-fontawesome', "//maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css", '', '', '');

        $subscribed = get_option(SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[SWIFTHD_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[SWIFTHD_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <?php } ?>
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="shd_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_shd_dashboard_subscribe">
                    ...
                    <input type="hidden" name="formid" value="643" id="formid" />
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('https://SwiftCloud.AI/support/tag/helpdesk/feed');
                                $maxitems = 0;
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function shd_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(SWIFTHD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function shd_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTHD_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(SWIFTHD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function shd_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str($_POST['data'], $subscribe_form_data);

            update_option(SWIFTHD_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(SWIFTHD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
